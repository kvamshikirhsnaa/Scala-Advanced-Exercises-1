  ...
  test("Car should be covariant in its type parameter") {
    ...
    object Car {
      private class CarImpl[T <: TransportableItem](items: Vector[T]) extends Car[T] {
        ...
        override def numberOfItems: Int = items.size
        override def item(n: Int): Option[T] = items.drop(n).headOption
      }

      def empty[T <: TransportableItem]: Car[T] = new CarImpl(Vector.empty)
    }
